...
const keywordSpottingApi = {
  createKeywordGroup: async (groupName) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/keyword-groups`, { group_name: groupName });
  ...
  createKeywordInGroup: async (groupId, keyword, synonyms) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/keyword-groups/${groupId}/keywords`, {
        keyword: keyword,
        synonyms: synonyms || []
      });
      return response.data;
  ...
  getKeywordsInGroup: async (groupId) => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/keyword-groups/${groupId}/keywords`);
      return response.data;
    }catch (error) {
  ...
  spotKeywords: async (keywords, caseSensitive, wholeWord, synonyms, model) => {
    ...
    try {
        const response = await axios.post(`${API_BASE_URL}/api/keyword-spotting`, JSON.stringify(requestBody), {
            headers: {
