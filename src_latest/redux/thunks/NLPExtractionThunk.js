...
export const grammerCorrectionThunk = createAsyncThunk(
    ...
    async (data, { dispatch }) => {
        console.log(" After calling grammerCorrectionThunk text", data);
        try {
            dispatch(setLoading(true));
            const response = await NLPExtractionAPI.grammerCorrection(data);
            const parsedResults = JSON.parse(response.worker_response.result);
            console.log("******** grammerCorrectionThunk *******", parsedResults);
            dispatch(setGrammerCorrection(parsedResults));
        } catch (error) {
