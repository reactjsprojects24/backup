...
export const handleLogin = async (dispatch, email, password, rememberMe, navigate) => {
  try {
    ...
    if (response.status === 200 && response.data.access_token) {
      ...
      // console.log("Login Success userResponse:",  userId);
...
export const logoutUser = () => async (dispatch, getState) => {
  const { access_token } = getState().auth;  // read token from redux
  ...
  try {
    ...
    dispatch(logout()); //clear redux state on logout
  } catch (error) {
    console.log(error, " failed to get  logout ");
  } finally {
    // always clear redux state, no matter what
    dispatch(logout());
  }
}
