...
export default function Keywordspotting() {
    ...
    // const filesByFolder = useSelector((state) => state.audio.filesByFolder?.upload_files || []);
    const filesByFolder = useSelector((state) => state.audio.filesByFolder || []);
    ...
    return (
        <>
            {/* This below code is with folder path as of now not in use */}

            {/* <div style={{ padding: '20px' }}>
                {filesByFolder && Array.isArray(filesByFolder) && filesByFolder.length > 0 ? (
                    <div style={{
                        overflowY: 'auto', height: '400px', '&::-webkit-scrollbar': {
                            display: 'none'
                        },
            ...
            {/* Add width: 100% to the table to fill its container */}
            ...
            {/* ****************************************************************** */}
            {/* This below code is with file path for temporary in use*/}
            <div style={{ padding: '20px' }}>
                {filesByFolder && Array.isArray(filesByFolder) && filesByFolder.length > 0 ? (
                    <div style={{ overflowY: 'auto', height: '400px' }}>
                        <table cellPadding="8" cellSpacing="0" style={{ width: '100%' }}>
                            ...
                            <tbody>
                                {filesByFolder.map((file, idx) => (
                                    <tr key={idx} style={{ fontSize: '11px' }}>
                                        <td><Checkbox color="primary" /></td>
                                        <td>{file.filename}</td>
                                        <td>{file.language}</td>
                                        <td>DD MM YYYY</td>
                                        <td>{Math.floor(file.metadata.duration / 60)}m {file.metadata.duration % 60}s</td>
                                        <td>
