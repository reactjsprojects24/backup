...
const audioApi = {
  ...
  uploadAudio: async (file) => {
    ...
    try {
      const response = await axiosInstance.post(API.UPLOAD_FILE, formData) //, {
      //   // headers: {
  ...
  analyzeAudio: async (data) => {
    ...
    try {
      ...
      console.log('failed to post audio analyze', error)
      return null;
    }
  },
  ...
  getAudioFiles: async () => {
    try {
      const response = await axiosInstance.get(API.LIST_AUDIO_FILES, {
        headers: getAuthHeaders()
      }
      );
      console.log("response of list audio files ", response)
      return response.data;
    } catch (error) {
  ...
  // API for Analyze in the Available Audio tab
  analyzeAvailableAudio: async (data) => {
    console.log("**** data in api analyze ****", data);
    try {
      console.log("got Analyze api ", data);
      const response = await axios.post(API.AVAILABLEAUDIO_ANALYZE, data, {
        headers: getAuthHeaders(),
      });
      console.log(" response of Analyze in vailable ========", response.data);
      return response.data;
    } catch (error) {
      // handleApiError(error, 'failed to post audio analyze');
      console.log('failed to post audio analyze', error)
  ...
  // get Audio Files By Folder Api (for bulk and keyword spotting)
  getAudioFilesByFolder: async () => {
    try {
      // const response = await axios.get(API.LIST_AUDIO_FILES_BY_FOLDER);
      const response = await axios.get(API.LIST_AUDIO_FILES, {
        headers: getAuthHeaders()
      });
      console.log("GETTING AUDIO FILES :",response);
      return response.data;
    } catch (error) {
