...
import { clearMetadata } from '../../redux/slices/audioSlice';

const AudioUpload = ({ singleFileMode = false, folderMode = false, onFilesUploaded, showMetadata: propShowMetadata, showTabs: propShowTabs }) => {
    ...
    const metadata = metadataResponse?.upload_response?.metadata;
    console.log("After Extracting metadata :", metadata);
    ...
    const handleFileUpload = (selectedFiles) => {
        ...
        // Reset Redux metadata when new file selected
            ...
            <Grid2 container>
                <Grid2 size={{ sm: 6, md: 6, lg: 6 }}>
                    <Box sx={{ margin: '1.33em' }}>
                        <Typography variant="h8" sx={{ fontWeight: 'normal', color: '#848484' }}>{folderMode ? "Upload Audio Folder" : "Upload Audio File"}:</Typography>
                    </Box>
                    <Box sx={{ pl: '1.33em', pr: '1.33em' }}>
                        ...
                        >
                            ...
                            <Typography variant="subtitle2" sx={{ fontWeight: 'bold', fontSize: '16px', lineHeight: '24px' }}>Drag
                                & drop {folderMode ? "folder" : "file"} or <span className="browse">Browse</span></Typography>
                            <span className="formates">Supported formates : WAV and MP4</span>

                            {/* <input type="file" accept=".wav,.mp4" webkitdirectory="true" directory="" multiple={!singleFileMode} onChange={(e) =>
                            ...
                            <Box>
                                ...
                                <Grid2 container spacing={5}>
                                    <Grid2 item xs={12} md={6}>
                                        ...
                                        <Typography variant="body2">File size: {metadata.file_size}</Typography>
                                        <Typography variant="body2">Duration: {metadata.duration}</Typography>
                                        <Typography variant="body2">Codec: {metadata.codec}</Typography>
                                    </Grid2>
                                    <Grid2 item xs={12} md={6}>
                                        <Typography variant="body2">Samplerate: {metadata.sample_rate}</Typography>
                                        <Typography variant="body2">Bitrate: {metadata.bitrate}</Typography>
                                        <Typography variant="body2">Channels: {metadata.channel}</Typography>
                                    </Grid2>
                                </Grid2>
                            </Box>
                        )}
