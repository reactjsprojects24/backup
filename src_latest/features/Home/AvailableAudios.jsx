...
import { availableAnalyzeAudioThunk } from "../../redux/thunks/homeAnalyzeThunk";
...
const AvailableAudio = () => {
    ...
    const getAllFiles = useSelector((state) => state.audio.audioFiles);
    console.log("getFiles----------------------", getAllFiles);
    const fileList = Array.isArray(getAllFiles?.upload_files) ? getAllFiles.upload_files : [];
    // const fileList = Array.isArray(getAllFiles?.upload_files) ? getAllFiles.upload_files?.[0]?.file_id : [];

    console.log("fileList----------------------", fileList);
    const [currentPage, setCurrentPage] = useState(1);
    const [itemsPerPage] = useState(5);
    const [noiseClicked, setNoiseClicked] = useState(false);
    ...
    const handleAudioAnalysis = (fileId) => {
        console.log("fileId to analyze ------------------", fileId);
        const payload = { file_id: fileId }
        dispatch(availableAnalyzeAudioThunk(payload));
    }
                    ...
                    <table cellPadding="8" cellSpacing="0" style={{ width: '100%', height: '25%' }}> {/* Add width: 100% to the table to fill its container */}
                        <thead>
                            <tr style={{ fontSize: '12px', alignItems: 'center' }}>
                                ...
                                <th>Date</th>
                                <th>Duration</th>
                                <th>Noise-Cancelled File</th>
                                <th>Action</th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
                            {/* {Array.isArray(getAllFiles) && getAllFiles.map((file, idx) => ( */}
                            {fileList.map((file, index) => (
                                <tr style={{ fontSize: '11px', alignItems: 'center' }} key={index}>
                                    ...
                                    <td style={{ textAlign: "center", verticalAlign: "middle" }}>
                                        {/* Add the switch here */}
                                        {/* <RedSwitch
                                            // checked={file.noiseCancelled || false}
                                            onChange={(e) => console.log(`Switch for ${file.filename}:`, e.target.checked)}
                                        /> */}
                                        <Box
                                            sx={{
                                                borderRadius: "12px",
                                        ...
                                        >
                                            ...
                                            <FormControlLabel
                                                control={
                                                    <RedSwitch
                                                        checked={noiseClicked}       // your state controlling ON/OFF
                                                        onChange={handleNoiseToggle} // your handler function
                                                    />
                                                }
                                            />
                                        </Box>
                                    ...
                                    <td><Buttons className="action-button"
                                        ...
                                        // onClick={handleAudioAnalysis(file.file_id)}  //this calls the function immediately on render
                                        onClick={() => handleAudioAnalysis(file.file_id)}  //this will call when user click on button
                                    >Anaylze </Buttons>
                                    </td>
                                </tr>
