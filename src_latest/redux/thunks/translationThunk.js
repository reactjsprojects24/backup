...
export const translateTextThunk = createAsyncThunk(
    ...
    async (payload, { dispatch }) => {
        try {
            console.log("**** translateTextThunk payload *****", payload);
            dispatch(setLoading(true));
            const response = await translationApi.textTranslation(payload);
              const parsedResults = JSON.parse(response.worker_response.result);
            console.log("translateTextThunk response", response , " parsedResults ****************", parsedResults);
            dispatch(setTranslations(parsedResults));
        } catch (error) {
